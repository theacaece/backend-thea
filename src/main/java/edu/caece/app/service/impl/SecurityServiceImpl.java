...
public class SecurityServiceImpl implements SecurityService{
	...
	public void logAccess(Person person) {
		String message = format("Acceso %s para %s (%s)", person.isEntryAllowed() ? "autorizado" : "denegado", person.getFullName(), person.getDni());
	...
	public void logAccessThresholdNotMet(Person person) {
		String message = format("No se pudo autorizar a la persona, confianza menor al 60% para %s (%s)", person.getFullName(), person.getDni());
	...
	@Override
	@Transactional
	public void log(String module, String message, Person person, EventType eventType) {
		SecurityLog securityLog = new SecurityLog(module,message,person, eventType);
		securityLogRepository.save(securityLog);
		// TODO: FIXME: Avoid null values being logged
		log.info(format("%s - %s (Person:%s): %s", module, eventType, person, message));
	}
